(() => {
    ...
    da.init();
    function createHighlightLogic(items, intervalId, shouldPauseOnHover) {
        let currentIndex = 0;
        let prevIndex = items.length - 1;
        function highlightNextItem() {
            items[prevIndex].classList.remove("highlighted");
            items[currentIndex].classList.add("highlighted");
            prevIndex = currentIndex;
            currentIndex = (currentIndex + 1) % items.length;
        }
        intervalId = setInterval(highlightNextItem, 800);
        if (shouldPauseOnHover) items.forEach((item => {
    ...
    const itemsServices = document.querySelectorAll(".item-services");
    let intervalIdServices;
    createHighlightLogic(itemsServices, intervalIdServices, true);
    const achievementsItems = document.querySelectorAll(".achievements__item");
    let intervalIdAchievements;
    createHighlightLogic(achievementsItems, intervalIdAchievements, false);
    const whyUsItems = document.querySelectorAll(".why-us__item");
    let intervalIdWhyUs;
    createHighlightLogic(whyUsItems, intervalIdWhyUs, false);
    window["FLS"] = true;
    isWebp();
    menuInit();
    dropdownSearch();
